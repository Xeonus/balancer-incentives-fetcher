...
class CoinPriceData extends Component {
  ...
  async fetchData() {
    const url = "https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&ids=balancer%2Cmeta%2Cqi-dao%2Ctracer-dao%2Clido-dao%2Cvitadao%2Cmcdex%2Cpickle-finance%2Ctelcoin%2Cethereum%2Cunion-protocol-governance-token&order=market_cap_desc&per_page=100&page=1&sparkline=false%22";
            ...
            <TableBody>
              {rows.map((row) => (
                <TableRow key={row.name}>
                  ...
                  {row.marketCap === 0 ? <TableCell align="right">{'-'}</TableCell> : <TableCell align="right"><DynamicValueFormatter value={Number(row.marketCap).toFixed(2)} name={row.name} decimals={0}/></TableCell>}
                </TableRow>
              ))}
            </TableBody>
          </Table>
